...
5. **No misquoted scripture.** The sponsor may only use verses the app sends it, and the relay removes any verse id it didn't send. The Scripture Guide only chooses a reference; the server copies the words from the Bible files, and refuses passages the agent didn't read first. Bible text is converted by script from public-domain sources ([bible-data.md](bible-data.md)), never typed by hand. Chapters whose numbering doesn't match English are shown in English.
